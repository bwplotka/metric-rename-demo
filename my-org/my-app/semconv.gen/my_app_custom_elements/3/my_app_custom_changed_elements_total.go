...
func MustNewMyAppCustomChangedElementsTotal(reg prometheus.Registerer) *MyAppCustomChangedElementsTotal {
	return &MyAppCustomChangedElementsTotal{promauto.With(reg).NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Custom counter metric (1.2.0) for my app counting important elements. It serves as an example of a very important metric that everyone is using. Replacement to my_app_custom_elements_total~elements.counter",
